...
export async function POST(req: NextRequest) {
  ...
  if (!token || !chatId) return NextResponse.json({ error: "Telegram not set up. Go to Notifications → enter Bot Token + Chat ID → Save." }, { status: 400 });

  const score = scoreSignal(trader, position);
...
_${esc(score.reasons.slice(0, 2).join(" · ")|| "Signal generated")}_`;

  const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ chat_id: chatId, text, parse_mode: "MarkdownV2", reply_markup: { inline_keyboard: [[{ text: `Trade ${position.symbol} on Bybit`, url: `https://www.bybit.com/trade/usdt/${position.symbol}` }]] } }),
  });
